	...
	<div>
		<div class="container">
			<div class="handle-box">
               咨询师编号：<el-input v-model="username" placeholder="" class="handle-input mr10"></el-input>
                咨询师姓名：<el-input v-model="realName" placeholder="" class="handle-input mr10"></el-input>
...
const list = [['序号','用户名', '姓名', '性别','手机', '邮箱', '是否启用', '最后登录时间']];
const exportXlsx = () => {
	console.log('tablev',tableData.value)
    tableData.value.map((item: any, i: number) => {
        const arr: any[] = [i+1];
        arr.push(...[item.userName, item.name,item.sex, item.phone, item.email, item.startUsing,item.lastLandingTime]);
        list.push(arr);
    });
