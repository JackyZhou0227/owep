	...
	<div>
		<div class="container">
			<div class="handle-box">
               教师编号：<el-input v-model="username" placeholder="" class="handle-input mr10"></el-input>
                教师姓名：<el-input v-model="realName" placeholder="" class="handle-input mr10"></el-input>
				<el-button color="#1c84c0" :icon="Search" @click="handleSearch">查询</el-button>
                <el-button-group class="ml-4">
                    <el-button color="#0eb594"  @click="handleExport">导出</el-button>
			...
			<el-table ref="multipleTableRef"  :data="tableData"   style="width: 100%;" height="380" @selection-change="handleSelectionChange">
				...
				<el-table-column  prop="stuId" label="编号"  ></el-table-column>
				...
				<el-table-column label="操作" width="220" align="center" >
                    ...
                    <el-tooltip content="分配班级" placement="top" effect="light" >
						<el-button text :icon="School" class="blue" @click="allocationClasses(scope.$index,scope.row)" v-permiss="2"></el-button>
					</el-tooltip> 
		...
		<el-dialog :title="titleEdit" v-model="editVisible" width="30%" >
			<el-form label-width="70px">
				...
				<el-form-item label="">
					<el-checkbox v-model="form.checked" label="勾选,表示拥有查询签约未分班学生的权限" size="large" />
				</el-form-item>
			</el-form>
			<template #footer>
		...
		<el-dialog title="分配用户组" v-model="groupVisible" width="30%">
			...
			<template #footer>
				<span class="dialog-footer">
					...
					<el-button color="#0EB594" @click="saveGroup">分配</el-button>
				</span>
			</template>
		</el-dialog>
        ...
        <!-- 分配班级弹出框弹出框框 -->
		<el-dialog title="分配班级" v-model="classesVisible" width="30%">
			<el-form label-width="85px">
				<el-form-item label="姓名" >
					<el-input v-model="formGroup.name" disabled></el-input>
				</el-form-item>
			...
			<template #footer>
				<span class="dialog-footer">
					...
					<el-button color="#0EB594" @click="saveClasses">保存</el-button>
				</span>
			</template>
		</el-dialog>
	</div>
...
import { Delete, Edit, Search, Plus ,School} from '@element-plus/icons-vue';
...
//分配班级回显
const allocationClasses=(index,row)=>{
  console.log(index);
  formGroup.name = row.name
...
const list = [['序号','编号', '姓名','性别','手机', '邮箱', '是否启用', '最后登录时间']];
const exportXlsx = () => {
	console.log('tablev',tableData.value)
    tableData.value.map((item: any, i: number) => {
        const arr: any[] = [i+1];
        arr.push(...[item.id, item.name,item.sex, item.phone, item.email, item.startUsing,item.lastLandingTime]);
        list.push(arr);
    });
